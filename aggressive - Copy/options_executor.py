...
class OptionsExecutor:
    ...
    def _execute_debit_spread(self, trade):
        ...
        try:
            ...
            order = (OrderBuilder()
                ...
                .add_option_leg("BUY_TO_OPEN", long_leg["symbol"], qty)
                .add_option_leg("SELL_TO_OPEN", short_leg["symbol"], qty)
                .build())
            ah = self._get_account_hash()
            resp = self.client.place_order(ah, order)
            if resp.status_code in (httpx.codes.CREATED, httpx.codes.OK):
        ...
        if stype == "NAKED_LONG":
            ...
            try:
                ...
                order = (OrderBuilder()
                    ...
                    .set_session(Session.NORMAL)
                    .set_duration(Duration.DAY)
                    .add_option_leg("SELL_TO_CLOSE", long_leg["symbol"], long_leg.get("qty", 1))
                    .add_option_leg("BUY_TO_CLOSE", short_leg["symbol"], short_leg.get("qty", 1))
                    .build())
                resp = self.client.place_order(ah, order)
                if resp.status_code in (httpx.codes.CREATED, httpx.codes.OK):
    ...
    def get_live_positions(self):
        ...
        try:
            ah = self._get_account_hash()
            resp = self.client.get_account(ah)
