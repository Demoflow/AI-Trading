"""
LETF Swing System - Roth IRA Live Script v2 (Smart Timing).

Same smart-entry / smart-exit logic as letf_live.py (PCRA).
Entries wait for optimal intraday conditions; sectors re-scanned every 90 min.
"""
...
def run(live=False):
    setup_logging()
    logger.warning("=" * 60)
    logger.warning("LETF ROTH IRA SYSTEM — DISABLED")
    ...
    with open("config/letf_roth_config.json") as _f:
        config = json.load(_f)
    ...
    logger.info("=" * 60)
    logger.info("LEVERAGED ETF SWING - ROTH IRA v2 (Smart Timing)")
