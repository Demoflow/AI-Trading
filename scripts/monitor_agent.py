...
# ── Log patterns ───────────────────────────────────────────────────────────────
# Stock VWAP scalper log patterns (v7.0+)
_RE_SCALP_OPEN  = re.compile(r"Opened (\w+) (LONG|SHORT) (\d+) shares @ \$([0-9.]+)")
_RE_SCALP_WIN   = re.compile(r"SCALP WIN: (LONG|SHORT) (\w+) (\d+) shares \$([+\-\d,.]+)")
_RE_SCALP_LOSS  = re.compile(r"SCALP LOSS: (LONG|SHORT) (\w+) (\d+) shares \$([+\-\d,.]+)")
_RE_PARTIAL_EXIT = re.compile(r"PARTIAL EXIT: (\w+) sold (\d+) shares @ \$([0-9.]+) P&L=\$([+\-\d,.]+)")
_RE_HALF_EXIT   = re.compile(r"HALF EXIT .+: (\w+) (\d+) shares P&L=\$([+\-\d,.]+)")
# Legacy option patterns (kept for backward compat with old logs)
_RE_SCALP_OPEN_LEGACY  = re.compile(r"SCALP OPEN: (\w+) (\w+) \$")
_RE_SCALP_WIN_LEGACY   = re.compile(r"SCALP WIN: (\w+) (\w+) \$([+\-\d,.]+)")
_RE_SCALP_LOSS_LEGACY  = re.compile(r"SCALP LOSS: (\w+) (\w+) \$([+\-\d,.]+)")
...
_RE_DAILY_HALT  = re.compile(r"DAILY LOSS LIMIT", re.IGNORECASE)
